...
INTEGER :: ZBE_ITER_COUNT, ZBE_MAX_ITER
...
ZBE_ITER_COUNT = 0
ZBE_MAX_ITER = 10000  ! Safety limit to prevent infinite loops

DO WHILE (ANY(SETUP_PRESSURE_ZONES_INDEX==0))
   ZBE_ITER_COUNT = ZBE_ITER_COUNT + 1
   IF (ZBE_ITER_COUNT > ZBE_MAX_ITER) THEN
      IF (MY_RANK==0) WRITE(LU_ERR,'(A)') 'WARNING: ZONE_BOUNDARY_EXCHANGE iteration limit reached'
      EXIT
   ENDIF
