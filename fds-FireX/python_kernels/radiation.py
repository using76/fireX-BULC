...
if TRITON_AVAILABLE:
    ...
    def kfst4_kernel(
        ...
        kappa_idx = i + j * IBAR + k * IBAR * JBAR
        kappa = tl.load(kappa_ptr + kappa_idx, mask=mask, other=0.0)
        
        T4 = T * T * T * T
        kfst4 = kappa * 4.0 * sigma * T4
        tl.store(kfst4_ptr + kappa_idx, kfst4, mask=mask)
    ...
    def radiation_transport_kernel(
        ...
        """Compute radiation transport and QR"""
        pid = tl.program_id(0)
        offsets = pid * BLOCK_SIZE + tl.arange(0, BLOCK_SIZE)
        total_cells = IBAR * JBAR * KBAR
        mask = offsets < total_cells
        ...
        cell_idx = offsets
        kfst4 = tl.load(kfst4_ptr + cell_idx, mask=mask, other=0.0)
        extcoe = tl.load(extcoe_ptr + cell_idx, mask=mask, other=0.0)
        
        il_sum = tl.zeros([BLOCK_SIZE], dtype=tl.float32)
        angle_weight = 4.0 * 3.14159265359 / NRA
        
        for n in range(NRA):
            il_idx = n * KBAR * JBAR * IBAR + cell_idx
            il_val = tl.load(il_ptr + il_idx, mask=mask, other=0.0)
            il_sum += il_val
        
        qr = kfst4 - extcoe * il_sum * angle_weight
        tl.store(qr_ptr + cell_idx, qr, mask=mask)


def compute_radiation_gpu(
    tmp: np.ndarray,
    ...
    """
    Main entry point for GPU radiation computation
    Called from C bridge
    """
    try:
        if not torch.cuda.is_available():
            return _compute_radiation_cpu(tmp, kappa_gas, il, qr, extcoe, 
                                         scaeff, dx, dy, dz, sigma)
        ...
        if TRITON_AVAILABLE:
            kfst4_kernel[grid](
                tmp_t.data_ptr(), kappa_t.data_ptr(), kfst4_t.data_ptr(),
                sigma, IBAR=ibar, JBAR=jbar, KBAR=kbar, BLOCK_SIZE=BLOCK_SIZE,
            )
            
            radiation_transport_kernel[grid](
                il_t.data_ptr(), kfst4_t.data_ptr(), extcoe_t.data_ptr(), qr_t.data_ptr(),
                dx, dy, dz, IBAR=ibar, JBAR=jbar, KBAR=kbar, NRA=nra, BLOCK_SIZE=BLOCK_SIZE,
            )
        else:
            # PyTorch fallback
            T = tmp_t[:-1, :-1, :-1]
...
def test_kernels():
    ...
    status = compute_radiation_gpu(tmp, kappa_gas, il, qr, extcoe, scaeff, 0.1, 0.1, 0.1)
    
    print(f"Status: {status}, QR range: [{qr.min():.2e}, {qr.max():.2e}]")
    return status
