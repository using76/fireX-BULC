...
def check_gpu_available() -> bool:
    """Check if GPU (CUDA) is available - called from C bridge"""
    try:
...
def compute_radiation_gpu(
    ...
    try:
        ...
        if TRITON_AVAILABLE:
            ...
            kfst4_kernel[grid](
                tmp_t, kappa_t, kfst4_t,
                sigma, total_cells, tmp_stride_j, tmp_stride_k,
                IBAR=ibar, JBAR=jbar, BLOCK_SIZE=BLOCK_SIZE,
            )

            radiation_transport_kernel[grid](
                il_t, kfst4_t, extcoe_t, qr_t,
                total_cells, total_cells, angle_weight,
                NRA=nra, BLOCK_SIZE=BLOCK_SIZE,
            )
        else:
            # PyTorch GPU fallback (optimized for CUDA)
